...
import {
  ...
  addRemoteRoute,
  addRemoteToConfig,
} from '@nx/react/src/module-federation/ast-utils';
...
export function updateHostWithRemote(
  ...
  if (host.exists(moduleFederationConfigPath)) {
    ...
    const sourceCode = host.read(moduleFederationConfigPath).toString();
  ...
  if (host.exists(remoteDefsPath)) {
    const sourceCode = host.read(remoteDefsPath).toString();
    const source = tsModule.createSourceFile(
      moduleFederationConfigPath,
      sourceCode,
      tsModule.ScriptTarget.Latest,
  ...
  if (host.exists(appComponentPath)) {
    const sourceCode = host.read(appComponentPath).toString();
    const source = tsModule.createSourceFile(
      moduleFederationConfigPath,
      sourceCode,
      tsModule.ScriptTarget.Latest,
